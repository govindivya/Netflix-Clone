...
const Movie = ({ movie, reviews, rating }) => {
  ...
  return !session ? (
    ...
    <>
      ...
      <section className="relative  z-50 min-h-[calc(100vh+76px)] h-auto m-0 min-w-full max-w-full">
        <div className="absolute top-0 left-0 w-full h-[calc(100vh+76px)]">
          <Image
            src={
...
export async function getServerSideProps(context) {
  ...
  const [movieDetails, ReviewsData] = await Promise.all([
    ...
    fetch(
      `https://api.themoviedb.org/3/movie/${id}/reviews?api_key=${process.env.TMDB_KEY}&language=en-hi&append_to_response=videos`
    ),
  ]);
